				...
				for j in range(nextEntry[1]):
					...
					if taskListIndex<len(tasks):
						if len(tasks[taskListIndex])>1:
							...
							extraTime=0
						javascriptString="%s[%s,%s],"%(javascriptString,int(1000*(float(this[0])))+extraTime,tasks[taskListIndex][0])
						print "[%s,%s],"%(int(1000*(float(this[0])))+extraTime,tasks[taskListIndex][0])
						raw_input() 
